...
import { updateArticleStatus, addArticleNote } from "~/lib/mutations";

export const Route = createFileRoute("/articles/$articleId")({
  component: ArticleDetailPage,
  loader: async ({ params }) => {
    return getArticleById(params.articleId);
  },
});
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Attachments */}
          <Section title={`Attachments (${article.attachments.length})`} noPadding>
            {article.attachments.length === 0 ? (
              <div className="empty-state py-8">
                <FileText className="w-8 h-8 mb-2" style={{ color: "var(--fg-faint)" }} />
            ...
            <div className="px-4 py-3">
                  ...
                  {article.notes.map((note: any) => (
                    ...
                    >
                      ...
                      <p
                        ...
                        style={{ color: "var(--fg-muted)" }}
                      >
                        {note.createdBy || "Unknown"} · {formatDate(note.createdAt)}
                      </p>
                    </div>
                  ))}
                </div>
              ...
              <div className="flex gap-2 mt-3">
                ...
                <Button
                  ...
                  variant="primary"
                >
                  {isAddingNote ? <LoadingSpinner size="sm" /> : <Send className="w-4 h-4" />}
                </Button>
              </div>
            </div>
          </Section>
        ...
        <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <Mail className="w-4 h-4" style={{ color: "var(--fg-faint)" }} />
                    <a
          ...
          <Section title="Details">
            <div className="space-y-3">
              <DetailRow label="Submitted" value={formatDate(article.submittedAt || article.createdAt)} />
              <DetailRow label="Last Updated" value={formatDate(article.updatedAt)} />
              <DetailRow
                label="Tier"
